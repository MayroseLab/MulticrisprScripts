...
from TreeConstruction_changed import DistanceTreeConstructor
from TreeConstruction_changed import DistanceCalculator
...
def print_res_to_csvV4(res, input_sg_genes_dict, genesList, genesNames, path, homology = False):
	def do_it(f,candidate_lst):
		for candidate in candidate_lst:
			#write candidate seq
			f.write(candidate.seq)
			f.write("\n")
			for target in candidate.targets_dict.items():
				f.write(str(target))
				f.write("\n")
			
	CSV_file_name = path+ "/output_CSV4.CSV"
	f = open(CSV_file_name, "w")
	if homology:
		for subgroup in res:
			do_it(f,subgroup.candidate_lst)
...
def print_res_to_csvV2(res, input_sg_genes_dict, genesList, genesNames, path, homology = False):
	def do_it(res, input_sg_genes_dict, genesList, genesNames, f):
		...
		for candidate in res:
			...
			for gene in genes:
				...
				if len(sites) > 0:
					seen_sites = dict() #for presenting sites redundencies. key: the site sequence; value: directions(sense/R) and num on occurrences
					site_index = 1
					for site in sites.keys():
						...
						if site == gene:
							...
							for item in sites[site]:  #item here is a single match site
								...
								##
								#find pos:
								if item[0] in seen_sites:
									directions_lst = seen_sites[item[0]]
								else:
									directions_lst = [0,0]
								sg_position = str(genesList[geneIndex].find(item[0], directions_lst[0])) #item[0] is the target site seq
								if sg_position == "-1":
									sg_position = str(CasSites.give_complementary(genesList[geneIndex]).find(item[0], directions_lst[1])) + "R"
									#update_seen_sites_dict(seen_sites, site, 1)
								#else:
								#	update_seen_sites_dict(seen_sites, site, 0)
	...
	f.close()
def update_seen_sites_dict(d, site_seq, direction):
	'''
...
def bottem_up(node, current_sg_genes_dict, current_genes_sg_dict, sgList, sgNames, Omega):
	...
	if len(current_genes_sg_dict) < 2 :  #only one gene
		...
		current_best_perm, lowest_of_widest = bottemsUpAlgorithm.find_best_sg_for_single_gene(leaf.name, sgList )#lowest_of_widest is not in use in this function
	else:
		#get the set cover from the bottem up algorithm
		current_res = bottemsUpAlgorithm.call_it_all(sgList, sgNames, current_sg_genes_dict, Omega)##call_it_all(sgList, sgNames, input_sg_genes_dict, Omega)## Naive.find_Uno_sgRNA(current_genes_sg_dict, Omega) #current best perm is a tuple with the perm and metedata of this perm
...
def call_it_all_wighted(genesList, genesNames, input_sg_genes_dict, input_genes_sg_dict, Omega, protodist_outfile, pylip_temps_path):
	...
	bottemsUpAlgorithm.fill_leaves_sets(upgmaTree)  # as apposed to the intermediate algorithem, here leaves are genes
	...
	best_permutations_DS = bottemsUpAlgorithm.call_it_all(sgList, sgNames, input_sg_genes_dict, Omega, df_targets)##call_it_all(sgList, sgNames, input_sg_genes_dict, Omega)## Naive.find_Uno_sgRNA(current
...
def call_it_all(genesList, genesNames, input_sg_genes_dict, input_genes_sg_dict, Omega, protodist_outfile, pylip_temps_path, df_targets, cfd_dict = None, PS_number = 12):
	...
	sgList = list(input_sg_genes_dict.keys())
	sgNames = copy.deepcopy(sgList)
	best_permutations_DS = bottemsUpAlgorithm.call_it_all(sgList, sgNames, input_sg_genes_dict, Omega, df_targets, cfd_dict, PS_number)##call_it_all(sgList, sgNames, input_sg_genes_dict, Omega)## Naive.find_Uno_sgRNA(current
