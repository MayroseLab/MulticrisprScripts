...
#def sub_tree_display(path, results_url, candidates_lst, f, consider_homology, counter, genes_names, genes_lst):
	...
	if consider_homology == True:
		...
		if use_thr == 0:
			...
			if not results_url:
				f.write("The minimal set of sgRNAs that could target the entire gene set with high efficiency")
			else:
		...
		if genomeAssembly != "Not_selected" and consider_homology == False:
			f.write("<br><br>*See off-targets search results <a href=\""+crista_url+"\" target=\"_blank\" style=\"font-size:16px; text-decoration: underline;\">here</a>.</font><br><br>\n")
		else: 
	...
	for c in candidates_lst:
		...
		n_candidate += 1
		if n_candidate < 6 and genomeAssembly != "Not_selected" and consider_homology == False:
			crista_label = "CRISTA*"
			crista_url = "http://crista.tau.ac.il/results/"+runId+"_crispys/output.php"
		else:
			crista_label = "CRISTA"
...
def tree_display(path, consider_homology = False, data = 'regular', genomeAssembly = 'Not_selected', use_thr=0):
	...
	if data == 'greedy_set_cover':
		...
		f = open(filepath, 'w')
		results_url = multicrispr_url+"/results1.html?jobId="+runId+"&removeRepetitions=0";
	else:
		candidates_lst = pickle.load(open(path + "/res_in_lst.p", "rb"))
		filepath = path + "/the_table.html"
		f = open(filepath, 'w')
		results_url = multicrispr_url+"/results1.html?jobId="+runId+"&removeRepetitions=1";
	genes_names = pickle.load(open(path + "/genesNames.p", "rb"))
	genes_list = pickle.load(open(path + '/genesList.p', 'rb'))

	# create crispys page
	if genomeAssembly != "Not_selected" and data == 'regular' and consider_homology == False:
		...
		if consider_homology == False:
			...
			for subgroup_item in candidates_lst:
				n_candidate += 1
				if len(subgroup_item.candidate_lst) == 0:
					continue
				c = subgroup_item.candidate_lst[0]
				if n_candidate > 1:
					sgrna_list += ','
				sgrna_list += c.seq
		...
		if sgrna_list:
			...
			qsub_path = create_job_file("CRISPYS_"+runId, cmd, "qsub_crista.sh", path, path)
			import os
			os.system('ssh bioseq@lecs2 qsub -l bioseq '+qsub_path)
