...
def parse_sam_out_table(samtable_filepath, sgrna_seq, genomic_db="hg19"):
	...
	while alter and not alter.isspace():
		#parse token
		...
		extended100_genomic_seq = fetch_dna_coordinates_offline(genomic_db, chromosome_num,
																start_pos - EXTENSION,
																end_pos + EXTENSION)
		if "N" in extended100_genomic_seq:
			if not is_first_line:
...
def run_bwa(sgRNA_seq, genomic_db="hg19", n_gaps=N_GAPS,
	...
	score_df = CRISTA_main.predict_crista_score(features_mat, logger)
...
def main(sgrna_seq, output_path, genome_assembly="solanum_lycopersicum"):
	global RESULTS_PATH
...
if __name__ == '__main__':
	...
	parser.add_argument('--genome_database', '-g', default="solanum_lycopersicum")
