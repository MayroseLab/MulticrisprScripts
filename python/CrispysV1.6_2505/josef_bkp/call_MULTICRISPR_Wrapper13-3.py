...
def CRISPys_main(fasta_file, path , alg = 'A', where_in_gene = 1, use_thr = 0,  Omega = 1, df_targets = Metric.cfd_funct, protodist_outfile = "outfile", min_length= 20, max_length = 20,start_with_G = False, internal_node_candidates = 10, internal_node_candidates_represent=10, PS_number = 12, PAMs = 0, off_targets = 0):
	...
	make_tree_display.tree_display(path,alg == 'E', data = 'removed_repetitions', genomeAssembly = off_targets, use_thr = use_thr)
	time_file = open("time.txt", 'w')
	time_file.write(str(stop - start))
	time_file.close()
	#genomeAssembly = 'Not_selected', use_thr=0
			...
			if num_of_covered_j == num_of_covered_i:
				...
				if cover_prob_j == cover_prob_i:
					if sum_num_of_mm_in_lowest_mm_target_in_relevant_genes(candidate_j, Omega, genes_list) > sum_num_of_mm_in_lowest_mm_target_in_relevant_genes(candidate_i, Omega, genes_list) : #total num of mm over best site in relevant genes
						return True
				#if cover_prob_j > cover_prob_i or (cover_prob_j == cover_prob_i and candidate_i.cut_expectation < candidate_j.cut_expectation):
				#	print('contained')
				#	return True
...
def remove_repetitions_in_targets_sites_(candidates_lst, alg, use_thr, Omega):
	'''older version - to premisive
	keep only with at least 1 unique targets'''
...
def parse_arguments(parser):
	...
	parser.add_argument('--i', type=int, default= 200, help='when choosing the consider homology option, this is the number of sgRNAs designed for each homology sub-group. Default: 200')
	parser.add_argument('--i2', type=int, default= 10, help='when choosing the consider homology option, this is the number of sgRNAs for each homology sub-group presented in the server. Default: 10')
	...
	parser.add_argument('--off_targets', type=str, default='Not_selected', help='the maximal number of possible polymorphic sites in a target. Default: 12')
...
if __name__ == "__main__":
	...
	CRISPys_main(fasta_file = args.fasta_file,
				 ...
				 internal_node_candidates_represent=args.i2,
				 PS_number = args.ps,
				 PAMs = args.PAMs,
