...
import WrapperVers3 as original_wrapper
import call_MULTICRISPR_Wrapper
#from importlib.machinery import SourceFileLoader

#original_wrapper = SourceFileLoader("module.name", "/groups/itay_mayrose/galhyams/MULTICRISPER/codeV1.1SetCover/WrapperVers3.py").load_module()
import itertools
...
def call_it_all(genesList, genesNames, input_sg_genes_dict, input_genes_sg_dict, Omega, protodist_outfile, pylip_temps_path, df_targets, internal_node_candidates, cfd_dict = None, PS_number = 12):
	upgmaTree, distance_matrix = original_wrapper.return_UPGMA(genesList, genesNames, protodist_outfile, pylip_temps_path) #to uncomment when using wighted
	write_newik_to_file(upgmaTree.root, pylip_temps_path)
	tree_to_file(upgmaTree.root, pylip_temps_path)
	bottemsUpAlgorithm.fill_leaves_sets_genes_tree(upgmaTree)  # as apposed to the intermediate algorithem, here leaves are genes
	...
	for leaf in node.node_targets_DS: ##leaf here is a gene. taking only the relevant genes
		...
		for sg in current_genes_sg_dict[leaf.name]:
			...
			if sg not in sgList:
				sgList.append(sg)
				sgNames.append(sg)
	if len(node.node_targets_DS) < 11 and len(node.node_targets_DS) > 1: #should change the first parameter to at least 10
		best_permutations_DS = bottemsUpAlgorithm.call_it_all(sgList, sgNames, current_sg_genes_dict, Omega, df_targets, cfd_dict, PS_number)##call_it_all(sgList, sgNames, input_sg_genes_dict, Omega)## Naive.find_Uno_sgRNA(current)
		if not (best_permutations_DS):
			return
